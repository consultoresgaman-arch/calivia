...
export default function QuickCheckIn({ userId, onDismiss }: Props) {
  ...
  return (
    <div className="qci-card anim-fade">
      <button className="qci-close" onClick={onDismiss} type="button" aria-label="Cerrar">
        <X size={14} strokeWidth={2} />
      </button>
      {savedMood ? (
        <p className="qci-thanks">Gracias por contarme. {MOOD_EMOJI[savedMood]}</p>
      ) : (
        <>
          <p className="qci-label">¿Cómo te sientes justo ahora?</p>
